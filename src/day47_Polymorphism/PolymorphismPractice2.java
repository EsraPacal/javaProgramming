...
public class PolymorphismPractice2 {
    ...
    public static void main(String[] args) {
        ...
        for (Employee scrumMember : scrumMembers) {
            if(scrumMember instanceof Tester){      // reference type is Employee. but object type is Tester
                testers.add((Tester) scrumMember);   // Employee scrumMember ====> Employee is the parent of the Tester, we need to cast
            }  // parent can be reference to the child, but child can not. so, we need the casting operation in order to add the testers in to the Tester ArrayList

            if(scrumMember instanceof Developer){
                developers.add((Developer) scrumMember);   // Since the reference type is Employee and the object type is Devp, We need to cast it
            }
